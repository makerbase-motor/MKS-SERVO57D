...
** 2. If the setting is successful, the LED light will flash slowly, and you can check the set working mode through the Mode option of the screen menu
...
void loop() {
  ...
  setMode(1,5); //Slave address=1, mode=5 (SR_vFOC)

  ackStatus = waitingForACK(5);      //Wait for the motor to answer

  if(ackStatus == 1)        //successfully set
  {
    while(1)    //Slow flashing, indicating that the setting is successful (you can view the set working mode in the Mode option of the screen menu)
    {
